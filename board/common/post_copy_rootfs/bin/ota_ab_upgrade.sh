...
# 3. 目标槽
echo "[3/7] Determining target slot..."
...
# 4. 管道直写（tar → dd，不落盘）
echo "[4/7] Writing to slot $TARGET..."
...
# 5. 读回校验
echo "[5/7] Verifying written data..."
...
# 6. 持久化期望哈希到 /etc/ota_manifest（供 ota_check verify 校验分区）
echo "[6/7] Writing ota_manifest..."
MANIFEST=/etc/ota_manifest
{
    echo "# OTA manifest — 由 ota_ab_upgrade.sh 生成，勿手改"
    echo "SLOT=$TARGET"
    echo "VERSION=$(cat "$TMPD/VERSION")"
    awk '$2=="rtt_system.bin"{print "RTT_SHA256="$1} $2=="linux_system.bin"{print "LINUX_SHA256="$1} $2=="rootfs.ext4"{print "ROOTFS_SHA256="$1}' "$TMPD/checksum.sha256"
} > "$MANIFEST" || error_exit "manifest write failed"
cat "$MANIFEST"

# 7. 事务性 env
echo "[7/7] Setting boot_slot=$TARGET..."
...
echo "Run 'reboot' now."
echo "Verify later with: ota_check verify $TARGET"
